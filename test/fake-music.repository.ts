...
export class FakeMusicRepository implements MusicRepository {
  ...
  constructor() {
    this.musics.push(
      new Music('1', 'In the name of love', 'Bonga', 'love', 2000, 10000),
      new Music('1', 'Love stronger', 'bonga', 'bonga', 1999, 1000),
  ...
  async search(filter: Filter): Promise<Music[]> {
    return this.musics.filter(
      (m) =>
        m.fullText.includes(filter.query) && m.fullText.includes(filter.artist),
    );
  }
}
